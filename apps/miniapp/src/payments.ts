...
import {api} from './api';
export async function payWithWechat(orderId:string,paymentChoice:'DEPOSIT'|'FULL_PAYMENT'|'BALANCE'|'ADDITIONAL',changeId?:string){
 ...
 const p=await api('/orders/'+orderId+'/wechat-pay',{paymentChoice,changeId});
 let cancelled=false;
 ...
 for(let i=0;i<4;i++){
  ...
  if(result.status==='SUCCEEDED'){await Taro.showToast({title:result.refundRequired||result.order.contractStatus==='PAYMENT_EXCEPTION'||result.order.contractStatus==='CANCELLED'?'款项已收到，正在退款':'微信付款已到账',icon:'none'});return;}
