...
export class PaymentService extends MvpService {
 ...
 async prepay(user:User,orderId:string,choice:unknown){
  const wx=this.gateway();ensure(['DEPOSIT','FULL_PAYMENT','BALANCE'].includes(String(choice)),'PAYMENT_CHOICE','请选择定金、全款或尾款',400);
  const p=await this.transaction(async tx=>{
   ...
   const d=obj(o.details),initial=choice!=='BALANCE';
   ensure(o.aftersaleStatus==='NONE'&&!d.pendingChange,'DISPUTED','请先处理订单售后或变更');
   ...
   else ensure(['ACCEPTED','FULFILLED'].includes(o.contractStatus)&&o.feeStatus==='AWAITING_PAYMENT'&&o.balanceFen>0,'NOT_PAYABLE','请先确认最终费用');
   const amountFen=choice==='DEPOSIT'?o.depositFen:choice==='FULL_PAYMENT'?o.totalFen:o.balanceFen;
   ...
   if(old){ensure(old.kind===choice&&old.amountFen===amountFen&&obj(old.rawEvent).openId===account.openId,'PAYMENT_PENDING','已有支付请求处理中，请继续原支付或等待关单');ensure(new Date(obj(old.rawEvent).expiresAt)>this.clock(),'PAYMENT_PENDING','正在关闭过期支付，请稍后重试');return old;}
      const payoutMode=d.payoutMode??(process.env.WECHAT_PROFITSHARING_ENABLED==='true'?'PROFITSHARING':'TRANSFER');
   this.validatePayoutMode(d,payoutMode);
   ...
   return tx.paymentRequest.create({data:{orderId,kind:choice as 'DEPOSIT'|'FULL_PAYMENT'|'BALANCE',amountFen,idempotencyKey:'wx:'+randomUUID(),provider:'wechat',rawEvent:{profitSharing:payoutMode==='PROFITSHARING',openId:account.openId,appId:wx.config.appId,mchId:wx.config.mchId,expiresAt:expiresAt.toISOString()}}});
  });
  let prepayId=obj(p.rawEvent).prepayId;
  if(!prepayId){const result=await wx.request('POST','/v3/pay/transactions/jsapi',{appid:wx.config.appId,mchid:wx.config.mchId,description:p.kind==='BALANCE'?'家厨上门服务尾款':'家厨上门服务预约',out_trade_no:p.id,time_expire:obj(p.rawEvent).expiresAt,notify_url:wx.config.notifyUrl,settle_info:{profit_sharing:obj(p.rawEvent).profitSharing===true},amount:{total:p.amountFen,currency:'CNY'},payer:{openid:obj(p.rawEvent).openId}});
  ...
  return {paymentId:p.id,amountFen:p.amountFen,...wx.clientParams(prepayId)};
 }
 async paymentStatus(user:User,id:string){
  ...
  const fresh=await this.db.paymentRequest.findUniqueOrThrow({where:{id}});return {paymentId:id,status:fresh.status,order:await this.getOrder(user,p.orderId)};
 }
 async paymentNotification(headers:Record<string,any>,raw:string){return this.recordPayment(this.gateway().notification(headers,raw));}
 async recordPayment(event:any){
  ...
  return this.transaction(async tx=>{
   ...
   let valid=p.kind==='BALANCE'?['ACCEPTED','FULFILLED'].includes(o.contractStatus)&&o.feeStatus==='AWAITING_PAYMENT'&&o.balanceFen===p.amountFen&&o.aftersaleStatus==='NONE'&&!d.pendingChange:o.contractStatus==='PENDING_PAYMENT'&&o.receivedFen===0&&new Date(d.paymentDeadline)>this.clock();
   const candidates:any[]=[];let coupon:any=null;
   if(valid&&p.kind!=='BALANCE'){
   ...
   if(p.kind==='BALANCE')o=await tx.order.update({where:{id:o.id},data:{balanceFen:0,feeStatus:'SETTLED',settlementStatus:o.confirmedAt?'PENDING':o.settlementStatus}});
   else{
   ...
   await this.audit(tx,o.customerId,'WECHAT_PAYMENT_SUCCEEDED',o.id,{paymentId:p.id,amountFen:p.amountFen});await this.event(tx,p.kind==='BALANCE'?'尾款到账':'首付款到账，等待厨师接单',o,candidates.slice(0,3).map(c=>c.chef.userId));
  });
 }
 ...
 async reconcilePayment(p:PaymentRequest){
  ...
  if(r.trade_state==='NOTPAY'&&(o.contractStatus==='CANCELLED'||new Date(obj(p.rawEvent).expiresAt)<=this.clock())){
 ...
 override async tick(){
  ...
  try{
   if(this.wx){
    ...
    await Promise.all(pending.map(async p=>{try{if(p.kind==='REFUND')await this.reconcileRefund(p);else await this.reconcilePayment(p);}catch(e){console.warn('WeChat reconciliation pending',p.id,e instanceof ApiError?e.code:'NETWORK_OR_DATABASE');}
